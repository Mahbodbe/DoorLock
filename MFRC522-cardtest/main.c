...
static uint8_t rc522_request(uint8_t reqMode, uint8_t *ATQA){
    ...
    rc522_write(BitFramingReg,0x00);             // <<< ?????? ?? 8-bit
...
static uint8_t rc522_anticoll_level(uint8_t level_cmd, uint8_t *out5){
    uint8_t cmd[2];
    uint8_t back[10]; uint8_t backBits=0, i;
    cmd[0] = level_cmd;
    cmd[1] = 0x20;
    rc522_write(BitFramingReg,0x00); 
    if(!rc522_transceive(cmd,2,back,&backBits)) return 0;
    if(backBits!=40) return 0;
...
while (1)
      {
        uint8_t atqa[2], uid[10], uid_len=0;

        if(rc522_request(PICC_REQIDL, atqa)){  
            uid_len = rc522_get_uid(uid);
            if(uid_len){
                lcd_clear();
                lcd_gotoxy(0,0); lcd_putsf("Card detected!");
