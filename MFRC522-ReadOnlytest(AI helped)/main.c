...
static uint8_t picc_halt(void){
    uint8_t cmd[4], crc[2], back[4], bits, ok;
    bits=0; ok=0;
    cmd[0]=PICC_HALT; cmd[1]=0x00;
    rc522_calc_crc(cmd,2,crc);
    cmd[2]=crc[0]; cmd[3]=crc[1];
    ok = rc522_transceive(cmd,4,back,&bits);
    /* HALT expects no reply; ignore ok/bits */
    return 1;
}
...
static uint8_t mifare_auth(uint8_t auth_cmd, uint8_t blockAddr, uint8_t *uid4, uint8_t *key6){
    ...
    rc522_write(CommandReg, PCD_Idle);
    set_bit_mask(FIFOLevelReg,0x80);
    rc522_write(FIFODataReg, auth_cmd);
    rc522_write(FIFODataReg, blockAddr);
    for(i=0;i<6;i++) rc522_write(FIFODataReg, key6[i]);
...
{
    uint8_t b7 = trailer[7];
    uint8_t b8 = trailer[8];
    uint8_t i  = (blockOffset & 0x03);

    /* Per NXP: C1 from B7[4+i], C2 from B8[4+i], C3 from B8[i] */
    *c1 = (uint8_t)((b7 >> (4 + i)) & 0x01);
    *c2 = (uint8_t)((b8 >> (4 + i)) & 0x01);
    *c3 = (uint8_t)((b8 >> (0 + i)) & 0x01);
    return 1;
}
...
{
    uint8_t code = (uint8_t)((c1<<2)|(c2<<1)|c3);
    /* 010, 001, 101, 111 => never write */
    return (code==2 || code==1 || code==5 || code==7) ? 1 : 0;
}
...
static uint8_t rc522_reselect(uint8_t *uid, uint8_t uid_len, uint8_t *sak){
    ...
    picc_halt();
    delay_ms(5);
    rc522_request(PICC_REQIDL, atqa);
    return rc522_select(uid, uid_len, sak);
}
...
void main(void){
    ...
    while(1){
        ...
        if(!rc522_request(PICC_REQIDL, atqa)) { delay_ms(120); continue; }
        ...
        /* Classic path: sector 1: block 4 (data) and block 7 (trailer) */
        if((sak&0xFC)==0x08 || (sak&0xFC)==0x18){
            ...
            /* Auth A for block 4 */
            if(!mifare_auth(MF_AUTH_KEY_A,4,uid4,DEF_KEY_A)){
            ...
            /* Re-select before trailer auth to avoid state issues */
            if(!rc522_reselect(uid, uid_len, &sak)){
                lcd_clear(); lcd_putsf("Re-select Fail");
                delay_ms(700);
                continue;
            }

            /* Try Trailer auth: first Key A, then Key B */
            if(!mifare_auth(MF_AUTH_KEY_A,7,uid4,DEF_KEY_A)){
                if(!mifare_auth(MF_AUTH_KEY_B,7,uid4,DEF_KEY_B)){
                    lcd_clear(); lcd_putsf("Auth Trailer Fail");
                    delay_ms(1000);
                    mifare_stop_crypto();
                    continue;
                }
